...
<html lang="pt-br">
  <head>
    ...
    <script>
        function remover(id)
        {
            location.href = 'removeVenda.php?acao=remover&id=' + id;
        }
    </script>
    ...
    <div class="container">
        ...
        <?php if (isset($_GET['remocao']) && $_GET['remocao'] == 1) { ?>
            <div class="bg-success pt-2 text-white d-flex justify-content-center">
                <h5>Venda deletada com sucesso</h5>
            </div>
        <?php } ?>
        ...
        <div class="container semborda">
            <h3>Remocão de Vendas</h3>
        </div>

        <div class="container">
            <?php foreach($vendas as $venda) { ?>

                <div class="col-sm-9" id="venda_<?= $venda->id ?>">
                    <?= $venda->id ?>
                    <?= $venda->id_cliente ?>
                    <?= $venda->id_produto ?>
                    <?= $venda->cliente ?>
                    <?= $venda->produto ?>
                    <?= $venda->qtdProduto ?>
                    <i class="fas fa-trash fa-lg text-danger" onclick="remover(<?= $venda->id ?>)"></i>
                </div>
            <?php } ?>
        </div>
